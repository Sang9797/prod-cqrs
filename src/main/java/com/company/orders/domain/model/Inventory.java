...
public class Inventory {
  ...
  public static Inventory create(String productId, String warehouseId, int initialQuantity) {
    ...
    return new Inventory(
        UUID.randomUUID().toString(), productId, warehouseId, initialQuantity, 0, Instant.now());
  }
  ...
  public void reserve(int qty) {
    if (qty <= 0) throw new IllegalArgumentException("Reserve quantity must be positive");
  ...
  public void release(int qty) {
    if (qty <= 0) throw new IllegalArgumentException("Release quantity must be positive");
  ...
  public void adjust(int delta) {
    ...
    if (newQty < 0) {
      throw new InsufficientInventoryException(
          String.format(
              "Adjustment of %d would make stock negative for product %s in warehouse %s",
