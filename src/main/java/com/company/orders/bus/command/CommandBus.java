 ...
 *
 * <p>Spring injects all CommandHandler beans as a List at startup. Each handler self-declares its
 * command type via commandType().
 *
 * <p>Virtual threads: because Tomcat uses virtual threads (spring.threads.virtual.enabled=true),
 * each dispatch() call already runs on a separate virtual thread automatically.
 */
@Component
