 ...
 *
 * <p>First request: client sends hash + full query → server stores the mapping. Subsequent
 * requests: client sends hash only → server looks up and executes. Unknown hash with no query →
 * returns PersistedQueryNotFound so client retries with full query.
 */
@Component
@Order(1)
