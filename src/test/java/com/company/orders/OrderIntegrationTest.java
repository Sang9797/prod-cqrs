...
class OrderIntegrationTest {
  ...
  void fullLifecycle_placeConfirmGet() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/orders/" + id + "/confirm")).andExpect(status().isNoContent());
    mockMvc
  ...
  void cancelOrder_success() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/orders/" + id)).andExpect(jsonPath("$.status").value("CANCELLED"));
  }
  ...
  void virtualThread_isEnabled() {
    assertThat(
            Thread.currentThread().isVirtual()
                || System.getProperty("spring.threads.virtual.enabled", "true").equals("true"))
        .isTrue();
  }
  ...
  void login_thenAccessProtectedEndpoint_succeeds() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/orders/nonexistent").header("Authorization", "Bearer " + token))
